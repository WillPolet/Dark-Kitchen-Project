            ...
            if (selector == null){
                ...
                buttonOne.addEventListener("click", () => {
                    if (document.getElementById(`shopInput${stringify(dishes[i].name)}`).value > 1){
                        ...
                        document.getElementById(`price${stringify(dishes[i].name)}`).textContent = "$ " + (parseInt(document.getElementById(`shopInput${stringify(dishes[i].name)}`).value) * dishes[i].price).toFixed(2)
                        // Set price there
